...
try {
    ...
    console.error("Telemetry server link disrupted:", err);
            ...
            setStats(prev => ({
                ...
                uptime: uptimeString,
                fps: computedFps,
                purity: isCommsFailure ? '0%' : ((backendData.data_purity_percent !== undefined) ? `${backendData.data_purity_percent}%` : '100.00%'),
                nasaPurity: isCommsFailure ? '0%' :((backendData.nasa_purity_percent !== undefined) ? `${backendData.nasa_purity_percent}%` : '100.00%'),
                terraPurity: isCommsFailure ? '0%' :((backendData.terra_purity_percent !== undefined) ? `${backendData.terra_purity_percent}%` : '100.00%'),
